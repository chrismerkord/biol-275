...
-   Identify when count data are appropriately modeled by a Poisson
    process.
-   State the assumptions of the Poisson model.
-   Explain how expected frequencies are generated under a Poisson null
    model.
-   Conduct and interpret a chi-square goodness-of-fit test for count
    data.
...
#| fig-cap: "Poisson probability distribution for λ = 1, 4, and 10. Points show $\\operatorname{Pr}[X = k]$ for integer values of $k$, connected by lines to highlight distribution shape. As $\\lambda$ increases, the distribution shifts right and becomes more symmetric."
#| fig-alt: "Plot of three Poisson probability mass functions with large colored points connected by lines. Lambda equals 1 is concentrated near zero, lambda equals 4 peaks around four, and lambda equals 10 peaks around ten and appears more symmetric."

# Poisson PMF plot with direct point styling (Okabe–Ito colors)
library(tidyverse)
...
dt <- tidyr::expand_grid(
  ...
  dplyr::mutate(
    p = dpois(k, lambda),
    lambda_f = factor(lambda, levels = lambdas)
  )

# Plot
ggplot(dt, aes(x = k, y = p)) +
  geom_line(aes(group = lambda_f), linewidth = 2.0, color = "grey55") +
  geom_point(
    aes(fill = lambda_f),
  ...
  labs(x = "k", y = "Pr[X = k]", fill = NULL) +
  coord_cartesian(expand = c(top = TRUE, left = TRUE, bottom=FALSE, right= TRUE)) +
