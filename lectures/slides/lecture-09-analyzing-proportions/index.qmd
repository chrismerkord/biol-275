...
![Two Wisconsin Fast Plants growing cups showing a binary outcome: one seed germinates, the other does not.](images/binomial-seed-germination.png){fig-alt="Two clear cup growing systems side by side. The left cup contains a sprouting plant labeled “Germinates,” and the right cup contains only soil labeled “Does Not Germinate,” with a large “vs.” between them." fig-align="center" width="600"}
:::
:::::
...
::: {.column width="50%"}
![Binomial model checklist: Binary? → Fixed n? → Independent? → Constant p?](images/checklist.png){fig-alt="A vertical checklist with four boxes connected by arrows: “Binary?”, “Fixed n?”, “Independent?”, and “Constant p?”." fig-align="center" width="333"}
:::
:::::
...
::: {.column width="40%"}
![Male and female wasps illustrating a binary outcome (male vs. female) in the wasp offspring example.](images/wasp-male-female-binary-example.jpg){fig-alt="Side-by-side image of two wasps labeled “Female” and “Male,” showing visible differences in body shape and size." fig-align="center" width="800"}
:::
:::::
...
$$
\operatorname{Pr}[6 \text{ left-handed flowers}] = \binom{27}{6} (0.25)^{6} (1-0.25)^{27-6}
$$

$$
= 296010 \times 0.000244 \times 0.002378
$$
...
-   *𝐻~0~*: The proportion $p$ of successes in the population is equal to $p_0$

-   *𝐻~𝐴~*: The proportion $p$ of successes in the population is not equal to $p_0$
...
-   𝐻~0~: The probability that a spermatogenesis gene falls on the X chromosome is 0.061 ( $p=0.061$ )

-   𝐻~𝐴~: The probability that a spermatogenesis gene falls on the X chromosome is something other than 0.061 ( $p\ne0.061$ )
:::
...
-   How many would be expected under *𝐻~0~*?
...
-   How likely are we to get 10 by chance alone if the *𝐻~0~* is true?

-   To decide, we need the null distribution, the sampling distribution for the test statistic assuming that *𝐻~0~* is true
...
-   If we use $\alpha=0.05$ then $p<\alpha$ so we reject the null hypothesis

**Conclude that the proportion** $p$ **of successes in the population is not equal to** $p_0$
:::
...
## Estimate the proportion of spermatogenesis genes on the X chromosme
