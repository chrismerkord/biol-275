...
Your task is to create a **horizontal bar chart** using ggplot2 functions that shows the **count of observations for each species**. Your plot must meet the following requirements:

-   Display species on the y-axis, with bar lengths representing counts.
-   Do not include a bar for observations with missing species values
-   Add a title to the plot

If you finish early, consider refining the plot by reordering species by frequency, adjusting labels for readability, or simplifying the legend.
...
-   Air temperature must be mapped to the **x-axis**.\
-   The histogram must use an explicitly chosen **bin width** (do not rely on the default).\
-   Both axis labels must be written in **sentence case** and include units where appropriate.

If you finish early, consider refining the plot by adjusting the bin width, adding a fill color with a subtle outline, or adding a vertical reference line (e.g., mean or medi
...
2.  Show your 4 graphs to an LA or instructor for a completion grade.
