...
3.  **Install required packages**
    ...
    install.packages("tidyverse", "palmerpenguins", "gapminder")
    ```
...
The goal of this section is not to memorize these parts, but to recognize them when you see ggplot code. Most plots you create will reuse the same pieces in different combinations.
...
> Handout Q:
>
> Record which variables are mapped to x and y on your worksheet.
...
In our example, we are plotting data from three penguin species. If we wanted to visualize which points represent each species, we can use an aesthetic mapping to link the `species` variable to the `color` aesthetic:
...
These default scales are usually sufficient, especially early in the course. Later, you will learn how and when to modify them intentionally.

This provides a default scale for color (i.e. a default set of colors). We can then modify those defaults by specifying a color scale ourselves. The viridis color scale is a common one that is color-blind friendly.
...
Facets create small multiples: the same graph repeated for different subsets of the data. Each panel shows the same variables using the same scales, making comparisons across groups easier.
