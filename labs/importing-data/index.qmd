...
1.  Pick up a physical copy of the lab [handout](handout.pdf "Download the Lab 3 Handout"), or print one if you are working outside of class.
2.  Log in to **Posit Cloud** and navigate to the **course workspace**.
3.  Open the Assignment Project named "lab-03-importing-data"
4.  In the console, install the required package **tidyverse** for this lab (only needs to be done once per project).
...
5.  Create a new R script and label it "lab-3-script".
6.  In your script, add this code and run it to load the tidyverse packge:
...
To read this dataset into R, use the `read_csv()` function. The file is stored in the project’s `data/` folder, so you must include the relative path when reading it. The file path must be surrounded by double quotes `"`. For example:
...
Using the example above, type the code to read in the `wnv-data.csv` file into your R script (not console).

After reading the file, inspect the resulting tibble to confirm:
