...
1.  The **tidyverse** package loads several other packages, including dplyr and ggplot2, which are used in the following code
2.  The **palmerpenguins** package contains the `penguins` dataset.
3.  Create an object named `penguins_clean` and give it the following value...
4.  Start with the `penguins` tibble from palmerpenguins, and...
5.  Pick the rows (`filter`) that do not (`!`) have a missing value (`NA`) in the `sex` column.
